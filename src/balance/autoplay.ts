...
import type { ContentPack, Profile, RunResult, RunState, SimEvent } from "../core/types";
...
function freeSlot(c: ContentPack, s: RunState): number | null {
  const ch = playableChapter(c, s.chapterIndex);
  const path = ch.map.paths[0]!.waypoints;
  const end = path[path.length - 1]!;
  let best: number | null = null, bestD = Infinity;
  for (let i = 0; i < ch.map.slots.length; i++) {
    if (s.towers.some(t => t.slotIndex === i)) continue;
    const slot = ch.map.slots[i]!;
    const d = Math.hypot(slot.x - end.x, slot.y - end.y);
...
function spendOnUpgrades(c: ContentPack, s: RunState): boolean {
  const sorted = [...s.towers].sort((a, b) => a.level - b.level);
...
function driveHero(c: ContentPack, s: RunState, events: SimEvent[]): void {
  const ch = playableChapter(c, s.chapterIndex);
  const path = ch.map.paths[0]!.waypoints;
  const hold = path[Math.max(0, path.length - 2)]!;
...
export function autoplayChapter(c: ContentPack, chapterIndex: number, opts: AutoplayOptions = {}): AutoplayReport {
  ...
  const useHero = opts.useHero ?? true;
  const profile = emptyProfile(opts.profile);
  const s = createRun(c, profile, chapterIndex);
  ...
  for (let w = 0; w < ch.waves.length; w++) {
    ...
    const towerLevels = s.towers.reduce((a, t) => a + t.level, 0);

    if (!startNextWave(s, c)) break;

    const events: SimEvent[] = [];
    let seconds = 0, stalled = false;
    while (s.phase === "wave") {
