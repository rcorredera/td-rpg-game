...
function basDuBloc(o: TileContentInput): number {
  const b: TileContentBox = composeTile(o);
...
describe("composition du contenu d'une tuile", () => {
  it("ne laisse jamais rien dépasser de la tuile", () => {
    for (const g of GABARITS) {
      for (const h of [120, 167, 220, 280, 350, 480]) {
        const o: TileContentInput = { ...g, h };
        const b: TileContentBox = composeTile(o);
        expect(b.iconCy - b.glyph / 2, `haut de l'icône, h=${h}`).toBeGreaterThanOrEqual(-h / 2);
        const bas: number = g.footerH > 0 ? b.footerTop + g.footerH : basDuBloc(o);
  ...
  it("agrandit l'icône avec la tuile, jusqu'à la résolution native", () => {
    ...
    const petite: number = composeTile({ ...SECONDAIRE, h: 120 }).glyph;
    const moyenne: number = composeTile({ ...SECONDAIRE, h: 167 }).glyph;
    const grande: number = composeTile({ ...SECONDAIRE, h: 240 }).glyph;
    ...
    // juste trop tôt.
    const bas: number = composeTile({ ...SECONDAIRE, w: 900, h: 150 }).glyph;
    const haut: number = composeTile({ ...SECONDAIRE, w: 900, h: 300 }).glyph;
  ...
  it("garde la jauge de pied au contact du bloc", () => {
    ...
    for (const h of [160, 240, 350, 480]) {
      const o: TileContentInput = { ...PRINCIPALE, h };
      const b: TileContentBox = composeTile(o);
      const ecart: number = b.footerTop - basDuBloc(o);
  ...
  it("respecte la marge imposée par l'habillage", () => {
    ...
    for (const g of GABARITS) {
      for (const minPad of [0, 10, 22, 30]) {
        const b: TileContentBox = composeTile({ ...g, minPad });
  ...
  it("centre le bloc dans sa zone utile", () => {
    for (const g of GABARITS) {
      const b: TileContentBox = composeTile(g);
      const bas: number = g.footerH > 0 ? b.footerTop + g.footerH : basDuBloc(g);
      const videHaut: number = (b.iconCy - b.glyph / 2) - (-g.h / 2 + b.pad);
      const videBas: number = (g.h / 2 - b.pad) - bas;
  ...
  it("occupe sa zone utile sur toute la gamme de tuiles du jeu", () => {
    ...
    for (const o of REELS) {
      const b: TileContentBox = composeTile(o);
      const bas: number = o.footerH > 0 ? b.footerTop + o.footerH : basDuBloc(o);
      const part: number = (bas - (b.iconCy - b.glyph / 2)) / (o.h - 2 * b.pad);
