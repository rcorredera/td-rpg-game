...
const WALKER: EnemyDef = { id: "walker", name: "W", lore: "", hp: 50, speed: 60, flying: false, goldReward: 1, damageToCastle: 1, meleeDps: 0 };
const FLYER: EnemyDef = { id: "flyer", name: "F", lore: "", hp: 50, speed: 60, flying: true, goldReward: 1, damageToCastle: 1, meleeDps: 0 };
const SNIPER: TowerLevelStats = { range: 700, damage: 999, fireRate: 5 };
...
describe("sim core", () => {
  ...
  it("refuse toujours une tour dont le content exige un déblocage", () => {
    ...
    const locked: ContentPack = mkContent({
      towers: {
    ...
    const s: RunState = createRun(locked, FRESH_PROFILE);
  ...
  it("ouvre les trois rangs de tour d'emblée, mais pas les spécialisations", () => {
    ...
    const maxRank = (p: Profile) => {
      const s: RunState = createRun(CONTENT, p);
      s.gold = 10000;
  ...
  it("fait PERDRE si un boss atteint le château", () => {
    ...
    const s: RunState = createRun(c, FRESH_PROFILE);
    parkHero(s);
    startNextWave(s, c);
    for (let i: number = 0; i < 60 * 40 && s.phase === "wave"; i++) tick(s, c, 1 / 60);
  ...
  it("une défaite après au moins une vague rapporte des Éclats (jamais punitif)", () => {
    ...
    startNextWave(s, CONTENT);
    s.phase = "defeat";
    const r: RunResult = computeResult(s, CONTENT);
  ...
  it("les Sceaux récompensent le TEMPS passé à retenir la horde, pas les kills", () => {
    ...
    const r: RewardRules = CONTENT.rewards;
    const s: RunState = createRun(CONTENT, FRESH_PROFILE);
    startNextWave(s, CONTENT);
  ...
  it("retire des Sceaux à chaque mort du héros", () => {
    ...
    s.phase = "defeat";
    const intact: number = computeResult(s, CONTENT).sceaux;
    s.heroDeaths = 2;
  ...
  it("le niveau de sort du profil change les stats du Tournoiement", () => {
    const lv1: WhirlwindLevel = CONTENT.hero.skills.whirlwind.levels[0]!;
    const lv3: WhirlwindLevel = CONTENT.hero.skills.whirlwind.levels[2]!;
    ...
    const s1: RunState = createRun(CONTENT, FRESH_PROFILE);
    const e1: EnemyState = mkEnemy(s1);
    castWhirlwind(s1, CONTENT, []);
    expect(e1.alive).toBe(true);
    expect(s1.heroKills).toBe(0);

    const s3: RunState = createRun(CONTENT, { ...FRESH_PROFILE, skills: { whirlwind: 3, rally: 1 } });
    expect(lv3.damage).toBeGreaterThan(lv1.damage + 1);
    const e3: EnemyState = mkEnemy(s3);
  ...
  it("createRun copie les niveaux de forge du profil", () => {
    const s: RunState = createRun(CONTENT, { ...FRESH_PROFILE, forge: { tower_archer: 2 } });
  ...
  it("spécialisation : réservée au niveau max, payée en or, définitive ; comptée à la revente", () => {
    const s: RunState = createRun(CONTENT, FULL_PROFILE);
    s.gold = 10000;
    buildTower(s, CONTENT, 0, "tower_archer", []);
    ...
    const before: number = s.gold;
    const specCost: number = CONTENT.towers["tower_archer"]!.specs!.find(sp => sp.id === "spec_volley")!.cost;
    ...
    const def: TowerDef = CONTENT.towers["tower_archer"]!;
    const invested: number = def.costs.reduce((a, b) => a + b, 0) + specCost;
  ...
  it("la brûlure (% PV max/s) achève une cible que l'impact ne tue pas", () => {
    ...
    const c: ContentPack = mkContent({ towers, enemies: { walker: { ...WALKER, hp: 100, speed: 5 } }, waves: [{ enemyId: "walker", count: 1 }], slots: [{ x: 50, y: 40 }] });
  ...
  it("Cœur de blizzard : ne tire plus mais ralentit en continu dans son aura", () => {
    ...
    const c: ContentPack = mkContent({ towers, enemies: { walker: { ...WALKER, hp: 100000 } }, waves: [{ enemyId: "walker", count: 1 }], slots: [{ x: 300, y: 40 }] });
    const s: RunState = createRun(c, FULL_PROFILE);
    s.gold = 1000;
    parkHero(s);
  ...
  it("un monstre renforcé explose plus fort sur le château (dégâts × √mult)", () => {
    ...
    const c: ContentPack = mkContent({ enemies: { walker: WALKER }, waves: [] });
  ...
  it("multi-chemins : un spawn avec pathIndex arrive par son propre chemin jusqu'au château", () => {
    ...
    const s: RunState = createRun(MINI, FRESH_PROFILE);
  ...
  it("groundOnly : une tour anti-sol ignore les volants, une tour polyvalente les abat (GDD)", () => {
    ...
    for (const [towerId, survives] of [["cannon", true], ["bow", false]] as const) {
      const c: ContentPack = mkContent({ towers, enemies: { flyer: FLYER }, waves: [{ enemyId: "flyer", count: 1 }], slots: [{ x: 300, y: 40 }] });
      const s: RunState = createRun(c, FRESH_PROFILE);
      parkHero(s);
  ...
  it("le héros bloque l'ennemi terrestre à portée de mêlée, jamais le volant", () => {
    for (const [enemy, blocked] of [[WALKER, true], [FLYER, false]] as const) {
      const c: ContentPack = mkContent({ enemies: { [enemy.id]: { ...enemy, hp: 100000 } }, waves: [{ enemyId: enemy.id, count: 1 }] });
      const s: RunState = createRun(c, FRESH_PROFILE);
  ...
  it("toute commande ciblée au tap agit dans le champ de bataille", () => {
    ...
    for (const p of OUT_OF_BOUNDS) {
      ...
      const s2: RunState = createRun(c, FRESH_PROFILE);
      s2.hasAccountSpell = true;
      const events: SimEvent[] = [];
      expect(castAccountSpell(s2, c, p, events)).toBe(true);
      const boom: SimEvent | undefined = events.find(e => e.type === "explosion");
  ...
  it("le givre ralentit : un ennemi gelé parcourt moins de distance", () => {
    const frost: Record<string, TowerDef> = {
      frost: {
        id: "frost", name: "G", lore: "", costs: [10],
    ...
    const run = (withTower: boolean) => {
      const c: ContentPack = mkContent({ towers: frost, enemies: { walker: WALKER }, waves: [{ enemyId: "walker", count: 1 }], slots: [{ x: 100, y: 40 }] });
      const s: RunState = createRun(c, FRESH_PROFILE);
      parkHero(s);
    ...
    const slowed: EnemyState = run(true), free: EnemyState = run(false);
  ...
  it("déterminisme (ADR-001) : mêmes commandes → état final identique, quel que soit le découpage du temps", () => {
    ...
    const play = (chunks: number[]) => {
      ...
      const o: Record<string, unknown> = JSON.parse(JSON.stringify(s)) as Record<string, unknown>;
      delete o["time"]; delete o["timeAcc"];
      for (const t of o["towers"] as { cooldown?: number }[]) delete t.cooldown;
      return JSON.stringify(o);
    };
    const a: string = play(Array(30).fill(1));      // 30 ticks de 1s
    const b: string = play(Array(60).fill(0.5));    // 60 ticks de 0.5s
  ...
  it("la forge augmente les dégâts des tours (un niveau = un kill plus rapide)", () => {
    ...
    const run = (forge: Record<string, number>) => {
      const c: ContentPack = mkContent({ towers, enemies: { walker: { ...WALKER, hp: 11, speed: 1 } }, waves: [{ enemyId: "walker", count: 1 }], slots: [{ x: 10, y: 40 }] });
      const s: RunState = createRun(c, { ...FRESH_PROFILE, forge });
      parkHero(s);
      buildTower(s, c, 0, "bow", []);
      startNextWave(s, c);
  ...
  it("sans tour, le château finit par tomber (simulation complète)", () => {
    ...
    startNextWave(s, CONTENT);
    for (let i: number = 0; i < 600 && s.phase !== "defeat"; i++) {
      tick(s, CONTENT, 0.5);
