...
import type { Insets, NineSlicePlan, PieceRect, StripPlan } from "./nineSlicePlan";
import {
...
describe("plan de découpe d'un nine-slice", () => {
  it("rogne à 1:1 quand le dessin d'angle est court", () => {
    ...
    const p: NineSlicePlan = planNineSlice(PAPER, 3, 16);
  ...
  it("garde la pièce entière et réduit quand le dessin couvre le coin", () => {
    ...
    const p: NineSlicePlan = planNineSlice(BUTTON, 37, 22);
  ...
  it("ramène toujours les marges à la taille visée", () => {
    ...
    for (const [nom, frame, detail] of PLANCHES) {
      for (const cible of [12, 16, 22, 24]) {
        const p: NineSlicePlan = planNineSlice(frame, detail, cible);
  ...
  it("prend chaque bande CONTIGUË au coin quand le coin est rogné", () => {
    ...
    for (const [nom, frame, detail] of PLANCHES) {
      const p: NineSlicePlan = planNineSlice(frame, detail, 22);
    ...
    for (const [nom, frame, detail] of PLANCHES) {
      for (const cible of [12, 16, 22, 24]) {
        ...
        for (const q of p.rects) {
          const colonne: boolean = [0, 1, 2].some(c => q.sx >= frame.left[c]! && q.sx + q.sw <= frame.right[c]!);
          const rangee: boolean = [0, 1, 2].some(r => q.sy >= frame.top[r]! && q.sy + q.sh <= frame.bottom[r]!);
  ...
  it("assemble sans trou ni recouvrement", () => {
    ...
    for (const [, frame, detail] of PLANCHES) {
      const p: NineSlicePlan = planNineSlice(frame, detail, 22);
      const couvert: Set<string> = new Set<string>();
      for (const q of p.rects) {
        for (let y: number = q.dy; y < q.dy + q.sh; y++) {
          for (let x: number = q.dx; x < q.dx + q.sw; x++) {
            const cle: string = `${x},${y}`;
  ...
  it("ramène les marges à ce que l'élément peut loger", () => {
    ...
    const voulu: Insets = { left: 22, right: 22, top: 22, bottom: 22 };
    for (const [w, h] of [[230, 30], [230, 44], [230, 58], [44, 44], [12, 9], [400, 300]] as const) {
      const i: Insets = fitInsets(voulu, w, h);
  ...
  it("rogne les deux côtés à parts égales", () => {
    ...
    const i: Insets = fitInsets({ left: 22, right: 22, top: 22, bottom: 22 }, 230, 30);
  ...
  it("compose une bande sans rogner ses embouts", () => {
    ...
    const b: StripPlan = planStrip({ left: [6, 130, 258], right: [16, 182, 268], top: 24, bottom: 38 });
  ...
  it("pave la bande sans trou ni recouvrement, et n'étire que son corps", () => {
    ...
    const corps: PieceRect = b.rects[1]!;
  ...
  it("ne réserve jamais plus de marge que la texture n'en possède", () => {
    ...
    const voulu: Insets = { left: 22, right: 22, top: 22, bottom: 22 };
    const i: Insets = sliceInsets(voulu, { w: 190, h: 61 }, { w: 48, h: 41 });
    ...
    for (const tex of [{ w: 48, h: 41 }, { w: 52, h: 52 }, { w: 20, h: 14 }]) {
      for (const box of [{ w: 190, h: 61 }, { w: 30, h: 30 }, { w: 600, h: 400 }]) {
        const f: Insets = sliceInsets(voulu, box, tex);
